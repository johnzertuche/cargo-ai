...
export default function App() {
  ...
  const finalizeProviderCleanup = async (grant: ProviderGrant) => {
    setBusy(true);
    setError("");
    try {
      const latest = await invoke<ProviderGrant>("finalize_provider_cleanup", { grantId: grant.id });
      setNotice(latest.status === "verified_revoked"
        ? "Provider revocation evidence was already complete. Local Keychain credential references were deleted and cleanup is now verified."
        : `Provider cleanup remains ${latest.status}.`);
      await refresh();
    } catch (caught) {
      setError(String(caught));
      await refresh();
...
function ProviderCleanupModal({ grant, busy, onClose, onFinalize }: { grant: ProviderGrant; busy: boolean; onClose: () => void; onFinalize: (grant: ProviderGrant) => Promise<void> }) {
  const [confirmed, setConfirmed] = useState(false);
  const dialogRef = useDialog(onClose, busy);
